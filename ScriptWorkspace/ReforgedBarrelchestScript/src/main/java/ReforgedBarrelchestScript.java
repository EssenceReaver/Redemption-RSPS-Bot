...
public class ReforgedBarrelchestScript implements Script {
    public static int afkAttempts = 0;
    public static Long lastUse = null;
    public static int barrelchestRetries = 0;
    public static boolean barrelRespawn = true;
    ...
    public int onLoop() {
        ...
        if (barrelchestRetries > 80){
            return -1;
        }

        NPC barrelchestNpc = NPC.getClosestNpc("Reforged Barrelchest");
        if (barrelchestNpc == null){
            barrelRespawn = true;
            barrelchestRetries++;
            return 250;
        }
        barrelchestRetries = 0;
        if (barrelRespawn){
            sleepSafe(new Random().nextInt(250,1500));
            barrelRespawn = false;
        }
        GameAction.attackNPC(barrelchestNpc.npcId);
