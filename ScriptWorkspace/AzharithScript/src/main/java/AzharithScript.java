...
public class AzharithScript implements Script {

    public static int afkAttempts = 0;
    public static Long lastUse = null;
    public static int azharithRetries = 0;

    public static final int PILLAR_OBJ = 180505;
    ...
    public int onLoop() {
        ...
        if ((System.currentTimeMillis() - scriptStartTime) / 1000 > 23700) {
            return -1;
        }

        if (azharithRetries > 80){
            return -1;
        }

        NPC azharithNpc = NPC.getClosestNpc("Azharith");
        GameObject pillarObj = GameObject.findClosestGameObjectById(PILLAR_OBJ);


        if (pillarObj != null){
            GameAction.interactWithObject(pillarObj.objectId, pillarObj.xPos, pillarObj.yPos);
            return 1000;
        }
        if (azharithNpc == null){
            azharithRetries++;
        ...
        azharithRetries = 0;
        GameAction.attackNPC(azharithNpc.npcId);
