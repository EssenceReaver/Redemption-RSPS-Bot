...
public class SlayerSuperiorScript implements Script {
    private static final int SUPERIOR_TELE_ID = 402189;
    private Integer superiorTeleQuantity;
    ...
    public int onLoop() {
        ...
        Integer currentQuantity = getSuperiorQuantity(2, 1000);
        if (currentQuantity == null){
            System.out.println("Could not find Superior Teleports in inventory");
            return -1;
        }

        if (superiorTeleQuantity == null) {
            superiorTeleQuantity = currentQuantity;
        }
        int currentKills = superiorTeleQuantity - currentQuantity;

        int npcNearPlayer = tryFindNPC("Slayer Superior", 14, 1000);
        if (npcNearPlayer > 0){
    ...
    private int teleportToBoss(){
        Integer teleIdx = LocalPlayer.getItemIndexFromInventory(SUPERIOR_TELE_ID);
        if (teleIdx == null){
            System.out.println("Could not find Superior Teleports in inventory");
            return -1;
        }
    ...
    private Integer getSuperiorQuantity(int attempts, int waitMs){
        for (int i = 0; i < attempts; i++) {
            Integer teleIndex = LocalPlayer.getItemIndexFromInventory(SUPERIOR_TELE_ID);
            if (teleIndex != null) {
