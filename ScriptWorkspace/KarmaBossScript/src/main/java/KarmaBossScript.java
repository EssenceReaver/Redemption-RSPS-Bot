...
public class KarmaBossScript implements Script {

    public static int afkAttempts = 0;
    public static Long lastUse = null;
    public static int armamosRetries = 0;
    ...
    public int onLoop() {
        ...
        if (armamosRetries > 80){
            return -1;
        }

        NPC armamosNpc = NPC.getClosestNpc("Armaros");
        if (armamosNpc == null){
            armamosRetries++;
            return 250;
        }
        armamosRetries = 0;
        GameAction.attackNPC(armamosNpc.npcId);
